      ...
      <ScrollView style={{ backgroundColor: c.background }} contentContainerStyle={[styles.content, { paddingTop: insets.top + 18, paddingBottom: insets.bottom + 30 }]}>
        ...
        {tickets.isLoading ? <Text style={[styles.body, { color: c.mutedForeground }]}>Caricamento ticket…</Text> : tickets.data?.map((ticket) => (
          <Pressable key={ticket.id} onPress={() => { setSelected(ticket); setStatus(ticket.status === 'accepted' || ticket.status === 'rejected' || ticket.status === 'in_progress' ? ticket.status : 'open'); }} style={[styles.card, { backgroundColor: c.card, borderColor: c.border }]}>
            ...
            {ticket.priority >= 100 ? <Text style={[styles.priority, { color: c.destructive }]}>PRIORITÀ ALTA</Text> : null}
...
const styles = StyleSheet.create({
   content: { paddingHorizontal: 20, gap: 12 }, header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, eyebrow: { fontFamily: 'Inter_700Bold', fontSize: 10, letterSpacing: 1.5 }, title: { fontFamily: 'Inter_700Bold', fontSize: 27 }, section: { fontFamily: 'Inter_700Bold', fontSize: 17, marginTop: 10 }, body: { fontFamily: 'Inter_500Medium', fontSize: 12, lineHeight: 18 }, summary: { borderRadius: 20, padding: 18 }, summaryValue: { fontFamily: 'Inter_700Bold', fontSize: 34 }, summaryLabel: { fontFamily: 'Inter_600SemiBold', fontSize: 12 }, card: { borderWidth: 1, borderRadius: 16, padding: 13, gap: 4 }, cardTitle: { fontFamily: 'Inter_700Bold', fontSize: 14 }, priority: { fontFamily: 'Inter_700Bold', fontSize: 10, letterSpacing: 1 }, statusRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 6 }, statusChip: { borderWidth: 1, borderRadius: 12, paddingHorizontal: 9, paddingVertical: 7 }, thread: { gap: 8, marginTop: 12 }, message: { borderRadius: 12, padding: 10 }, input: { borderWidth: 1, borderRadius: 12, minHeight: 48, marginTop: 12, padding: 10, fontFamily: 'Inter_500Medium', fontSize: 13 }, primary: { borderRadius: 12, paddingHorizontal: 16, paddingVertical: 12 }, primaryText: { fontFamily: 'Inter_700Bold', fontSize: 13 }, denied: { flex: 1, alignItems: 'center', paddingHorizontal: 28, gap: 14 },
});