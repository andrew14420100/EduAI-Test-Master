...
Installable Android icon verification requires more than Java: the host must expose a compatible Android SDK, `adb`, and an emulator or connected device for each API level. In this Replit Linux workspace, Nix can persist JDK 17, platform-tools, emulator runtime, and X11/QEMU libraries, while the local SDK root can hold platforms and build-tools.
